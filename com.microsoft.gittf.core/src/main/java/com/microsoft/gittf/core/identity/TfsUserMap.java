...
public class TfsUserMap
        extends UserMap {
    private static final String UNAUTHENTICATED_IDENTITY = "Microsoft.TeamFoundation.UnauthenticatedIdentity";
    private static final String CLAIMS_IDENTITY = "Microsoft.IdentityModel.Claims.ClaimsIdentity";
    private static final String WINDOWS_IDENTITY = "System.Security.Principal.WindowsIdentity";
    private static final String SCHEMA_PROPERTY = "SchemaClassName";
    ...
    protected Map<String, List<TfsUser>> findTfsUsers(
        ...
        log.info("TFS identites look-up");
        Check.notNull(IMS, "IMS");
                ...
                for (int k = 0; k < identitiesList.length; k++) {
                    ...
                    if (mappedIdentites != null && mappedIdentites.length > 0) {
                        ...
                        for (int j = 0; j < mappedIdentites.length; j++) {
                            ...
                            log.debug("Identity:      " + identity.getUniqueName());
                            log.debug("Identity Type: " + identity.getDescriptor().getIdentityType());
                            for (final Entry<String, Object> property : properties) {
                                log.debug(property.getKey() + " = " + property.getValue().toString());
                            }
                            ...
                            if (identity != null
                                ...
                                log.warn(MessageFormat.format(
                                        "Incorrect identity type \"{0}\" or schema class \"{1}\". Identity ignored.", type, schema));
                            }
                        }
